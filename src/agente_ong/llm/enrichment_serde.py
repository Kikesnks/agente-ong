...
Envuelve `report_serde.report_to_dict`/`report_from_dict` (ui/, T10 no se toca) y añade las
3 claves nuevas del filtro semántico. Vive en `llm/` — no en `ui/report_serde.py` — para que
...
def enriched_report_to_dict(enriched: EnrichedReport) -> dict:
    ...
    data["discarded"] = [opp_to_dict(o) for o in enriched.discarded]
    data["unclassified"] = [opp_to_dict(o) for o in enriched.unclassified]
...
def enriched_report_from_dict(data: dict) -> EnrichedReport:
    ...
    Retrocompatible con informes pre-R7 (persistidos antes de este cableado): sin las 3
    claves nuevas, `.get(..., default)` los trata como si el filtro nunca se hubiera
    aplicado — mismo patrón que `result_type`/R20 en `report_serde.py`.
    """
    base: ResearchReport = report_from_dict(data)
    return EnrichedReport(
        base=base,
        discarded=[opp_from_dict(o) for o in data.get("discarded", [])],
        unclassified=[opp_from_dict(o) for o in data.get("unclassified", [])],
